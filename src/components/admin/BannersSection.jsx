...
export default function BannersSection({ banners = [], onRefresh }) {
  ...
  const handleSave = async () => {
    if (editId) { await supabase.entities.Banner.update(editId, form); }
    else { await supabase.entities.Banner.create({ ...form, is_active: true }); }
    setDialogOpen(false); onRefresh();
  };
  const handleDelete = async (id) => { await supabase.entities.Banner.delete(id); onRefresh(); };
