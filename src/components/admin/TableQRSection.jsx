...
export default function TableQRSection({ restaurant, onRefresh }) {
  ...
  const { data: tables = [] } = useQuery({
    queryKey: ['tables'],
    queryFn: () => supabase.entities.TableMapping.list(),
  });
  ...
  const handleToggle = async (v) => {
    setEnabled(v);
    await supabase.entities.Restaurant.update(restaurant.id, { table_qr_enabled: v });
    onRefresh();
  };

  const addTable = async () => {
    if (!newTable.trim() || !newUrl.trim()) return;
    await supabase.entities.TableMapping.create({ table_number: newTable.trim(), qr_url: newUrl.trim() });
    setNewTable('');
    setNewUrl('');
  ...
  const deleteTable = async (id) => {
    await supabase.entities.TableMapping.delete(id);
    queryClient.invalidateQueries({ queryKey: ['tables'] });
  };
