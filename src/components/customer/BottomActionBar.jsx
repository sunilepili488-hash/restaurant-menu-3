...
export default function BottomActionBar({ restaurant, favoritesCount, cartCount, onFavoritesClick, onCartClick }) {
  ...
  const sendAlert = (label) => {
    ...
    sendSilently(restaurant, msg, { type: 'waiter_call', action: label, table: tableNum });
    ...
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 px-2 pb-1.5">
        <div className="glass rounded-2xl flex items-center justify-between px-5 py-2.5 shadow-lg">
          {/* Left: Bag button + count to its right */}
          ...
          <motion.button
            ...
            className="w-9 h-9 rounded-full bg-accent text-accent-foreground flex items-center justify-center flex-shrink-0"
          >
            <Bell className="w-4 h-4" />
          </motion.button>

          {/* Right: count + Favorites button to its right */}
          <div className="flex items-center gap-2">
            {favoritesCount > 0 && (
